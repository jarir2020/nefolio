...
if( route(2)  ==  "provider" ):
    $count          = $conn->prepare("SELECT * FROM services WHERE service_deleted = :del ");
    $count        ->execute(array("del"=>0));
    $count          = $count->rowCount();
    $services = $conn->prepare("SELECT * FROM service_api");
    $services->execute(array());
    $services = $services->fetchAll(PDO::FETCH_ASSOC);
    $active   = $_POST["active"];
    echo '<li'; if( !$active ): echo ' class="active"'; endif; echo '>
            <a href="/admin/services">All services ('.$count.')</a>
          </li>';
      foreach ($services as $service):
        echo '<li'; if( $service["id"] == $active ): echo ' class="active"'; endif; echo '>
                <a ';  echo ' href="admin/services?id='.$service["id"].'"><span class="label-id">'.$service["id"].'</span> '.$service["api_name"].' ('.countRow(["table"=>"services","where"=>["service_api"=>$service["id"],"service_deleted"=>0]]).')</a>
</li>';
      endforeach;
    exit();
  endif;
