...
elseif ($action == "delete_fake_users") :
$clients        = $conn->prepare("SELECT * FROM clients");
$clients        -> execute(array());
$clients       = $clients->fetchAll(PDO::FETCH_ASSOC);
    $i = 0;
    foreach ($clients as $client) :
        $registerDate = DateTime::createFromFormat("Y-m-d H:i:s", $client["register_date"]);
        $sevenDaysAgo = (new DateTime())->modify('-7 days');
        if ($registerDate < $sevenDaysAgo) {
            $delete       = $conn->prepare("DELETE FROM clients WHERE client_id=:id && balance=:balance && spent=:spent");
            $delete->execute(array("id" => $client["client_id"],"balance" => 0,"spent" => 0));
            $i++;
        }
     endforeach;
    if ($i > 0) { $res["title"] = "Successfull removed all fake users"; $res["icon"] = "success"; echo json_encode($res); exit();} 
    else { $res["title"] = "Fake users Not Found"; $res["icon"] = "error"; echo json_encode($res); exit();}
