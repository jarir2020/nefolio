...
if ($SERVICE_DETAIL["service_api"] == 0) {
    try {
        ...
        // Insert Manual Order
        $insert_manual_order = $conn->prepare("INSERT INTO orders SET 
            order_start=:count,
            order_profit=:profit,
            order_error=:error,
            client_id=:c_id,
        ...
        $execute_manual_order = $insert_manual_order->execute([
            "count"    => "0",
        ...
        if (!$execute_manual_order) {
            throw new Exception("Failed to insert manual order: " . json_encode($insert_manual_order->errorInfo()));
        }
        ...
        $execute_update_user = $update_user->execute([
        ...
        if (!$execute_update_user) {
            throw new Exception("Failed to update user balance: " . json_encode($update_user->errorInfo()));
        }

        // Insert Order Log
        $insert_order_log = $conn->prepare("INSERT INTO client_report SET 
            client_id=:c_id, action=:action, report_ip=:ip, report_date=:date");

        $execute_order_log = $insert_order_log->execute([
            "c_id"   => $API_CLIENT["client_id"],  // Fixed issue with undefined $user["client_id"]
            "action" => "New Manual Order #".$ORDER_ID." has been placed by ".$API_CLIENT["username"].".",
            "ip"     => GetIP(),
            "date"   => date("Y-m-d H:i:s")
        ]);

        if (!$execute_order_log) {
            throw new Exception("Failed to insert order log: " . json_encode($insert_order_log->errorInfo()));
        }
        ...
        if ($settings["alert_newmanuelservice"] == 2) {
            ...
            mail($settings['admin_mail'], "New manual orders", $msg);
        }
        ...
        // Rollback on Error
        $conn->rollBack();
        APIErrorExit("Error: " . $e->getMessage());
    }
}
